...
*/
    ...
    if (!reqObj && values) {
      ...
      // Make sure the model type is set for POST and PUT.
      if (restMethod === 'post' || restMethod === 'put') {
        if (!reqObj[config.modelTypeField]) {
          reqObj[config.modelTypeField] = config.resource;
          //_.extend(reqObj, {config.modelTypeField: config.resource});
        }
      }
